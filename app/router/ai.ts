...
export const generateThreadSummary = base
  ...
  .handler(async ({ input, context, errors }) => {
    ...
    const instructions = [
      ...
      "Use only the provided thread content; do no invent facts, names, or timelines.",
      ...
      "Style: neutral, specific and concise. Preserve terminology from the thread (names, acronyms). Avoid filler or meta-commentary. Do no add a closing sentence.",
    ...
    return streamToEventIterator(toUIMessageStream({ stream: result.stream }));
  });
...
export const generateCompose = base
  ...
  .handler(async ({ input }) => {
    ...
    const instructions = [
      ...
      "Output strictly in Markdown (paragraphs and optional bullet lists). Do not output HTML or images.",
